...
class SBOMEnhancer:
    ...
    def __init__(self, sbom_path: str):
        self.sbom_path = sbom_path#!/usr/bin/env python3
"""
Extract dependency graph from container image metadata for multiple ecosystems.
    ...
    def enhance_with_dependencies(
        ...
        for component in self.sbom.get('components', []):
            ...
            if purl:
                ...
                # Format: pkg:pypi/package-name@version or pkg:npm/package-name@version
                for eco_prefix in ['pkg:pypi/', 'pkg:npm/']:
                    if purl.lower().startswith(eco_prefix):
                        ...
                        if match:
                            ...
                            # Also map without scope for npm scoped packages
                            if purl_name.startswith('@'):
                                purl_map[purl_name.split('/')[-1]] = ref
        ...
        for component in self.sbom.get('components', []):
            ...
            for dep_name in deps:
                ...
                # For npm scoped packages, try without scope
                if not dep_ref and '/' in dep_name:
                    dep_ref = purl_map.get(dep_name.split('/')[-1])
